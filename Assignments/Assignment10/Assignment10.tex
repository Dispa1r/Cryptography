...
\begin{alphalist}
	...
	\newline 
	\newline {\bf Solution}: Yes, given \textit{h} it is hard to find a message having that digest $H(x) = \textit{h}$.
...
\begin{alphlist}
	...
	\newline
	\newline {\bf Solution:} Compute $r = \alpha^{k}$ (mod p). $r = 23^{31}$ (mod 97). $ r = 87$.
	...
	\newline\indent {\bf Solution:} Compute $v_{1} = \beta^{r} \cdot r^{s}$ (mod p) and $v_{2} = \alpha^{m}$ (mod p).
